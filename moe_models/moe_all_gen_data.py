...
def main():
    ...
    for size in [3000]:#, 5000, 8000]:
        dataset =  'expert_1_gate_1_single_shallow_multi_class_'+str(size)
        X, y, trainset, trainloader, testset, testloader, num_classes = data_generator.generate_data(dataset, size, 128, 6)
        
        runs = []
        for r in range(0, num_runs):
            models = run_experiment(dataset, single_model_shallow, trainset, trainloader, testset, testloader, num_classes, total_experts, epochs, loss_importance=0.5)
            runs.append(models)
