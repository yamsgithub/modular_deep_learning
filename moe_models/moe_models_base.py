...
class two_temp_optimizer:
    def __init__(self, optimizer_gate, optimizer_experts):
    ...
    def optimise(self, model=None, inputs=None, labels=None, 
        ...
        outputs_with_T = model(inputs, T=T)
...
#                    print('l_imp_reg', l_imp)
                ...
                if not T[epoch] == 1.0:
                   gate_probabilities_batch_high_T = optimizer.gate_probabilities_batch_high_T
...
#                 history['per_sample_entropy'].append(sample_entropies)
            ...
            if epoch > T_decay_start and T_decay > 0:
                if epoch+1 < len(T):
                    decay = T[epoch] * (1. / (1. + T_decay * (epoch-T_decay_start)))
                    if decay < 0.001:
                        decay = 0.001
                    T[epoch+1] = decay
