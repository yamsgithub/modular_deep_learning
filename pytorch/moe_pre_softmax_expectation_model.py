...
class moe_pre_softmax_expectation_model(nn.Module):
    ...
    def forward(self,inputs, T=1.0):   
        ...
        output = F.softmax(torch.sum(p*y, 1)/T, dim=1)
    ...
    def train(self, trainloader, testloader,
        ...
        history = {'loss':[], 'loss_importance':[], 'baseline_losses':[],
                   ...
                   'entropy':[], 'sample_entropy':[], 'EY':[],'mutual_EY':[], 'H_EY':[],'H_Y':[], 'H_E':[],
        ...
        for epoch in range(epochs):  # loop over the dataset multiple times
            ...
            for inputs, labels in trainloader:
                ...
                for index in range(0, self.num_experts):
                    acc = accuracy(F.softmax(self.expert_outputs[:,index,:], dim=1), labels, False)
                    ...
                    e_loss = loss_c(reduction='none')(F.softmax(expert_outputs[:,index,:], dim=1), labels)
            ...
            with torch.no_grad():
                ...
                for test_inputs, test_labels in testloader:
                    ...
                    for index in range(0, self.num_experts):
                        acc = accuracy(F.softmax(test_expert_outputs[:,index,:], dim=1), test_labels, False)
