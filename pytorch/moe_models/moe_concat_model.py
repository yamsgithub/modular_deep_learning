...
class moe_concat_model(nn.Module):
    ...
    def forward(self,inputs, T=1.0):
        ...
        for i, expert in enumerate(self.experts):
            ...
            if output.shape[-1] > max_classes:
                max_classes = output.shape[-1]
            y.append(output)
        print(max_classes)
        for i in range(len(y)):
            ...
            for j in range(len(y[i])):
                print(y[i][j])
                if len(y[i][j]) < max_classes:
                    y[i][j] = torch.cat([y[i][j], torch.zeros(max_classes-len(y[i][j]))])
                print(y[i][j], '\n')
        y = torch.stack(y)
        y.transpose_(0,1)
        ...
        output = torch.cat(p*y, 1)

        return output
    
    def train(self, trainloader, testloader, label_map,
        ...
        for epoch in range(epochs):  # loop over the dataset multiple times
            ...
            for inputs, labels in trainloader:
                ...
                for i in range(len(labels)):
                    labels[i] = label_map.index(labels[i])
                ...
                if T > 1.0:
                    loss, gate_probabilities_batch_high_T = moe_models.two_temp_optim(self, inputs, labels, outputs, T,
                    ...
                    if w_ideal_gate > 0.0 and self.num_experts > 1:
                        ...
                        for i in range(self.num_experts):
                            l_experts.append(loss_criterion(expert_outputs[:,i,:],labels, reduction='none'))
                ...
                for index in range(0, self.num_experts):
                    acc = accuracy(expert_outputs[:,index,:], labels, False)
                    ...
                    e_loss = loss_criterion(expert_outputs[:,index,:], labels, 'none')
                    e_sample_loss = torch.sum(gate_outputs[:, index].flatten()*e_loss)
            ...
            with torch.no_grad():
                ...
                #test_outputs_all = torch.tensor([])
                for test_inputs, test_labels in testloader:
               	    test_inputs, test_labels = test_inputs.to(device, non_blocking=True), test_labels.to(device, non_blocking=True)
                    for i in range(len(test_labels)):
                        test_labels[i] = label_map.index(test_labels[i])

                    test_outputs = self(test_inputs)
                    #test_outputs_all = torch.cat([test_outputs_all, test_outputs])
            ...
            with torch.no_grad():
                ...
                history['expert_sample_accuracy'].append((torch.div(expert_sample_train_running_accuracy,torch.sum(gate_probabilities, dim=0))).cpu().numpy())
                ...
                history['kl_div_gate'].append(moe_models.kl_divergence(gate_probabilities, torch.mean(gate_probabilities, dim = 0).repeat(len(gate_probabilities),1)).cpu().numpy())
            if T> 1.0:
                ...
                with torch.no_grad():
                    history['expert_sample_accuracy_T'].append((torch.div(expert_sample_train_running_accuracy_T,torch.sum(gate_probabilities_high_T, dim=0))).cpu().numpy())
                    ...
                    history['kl_div_gate_T'].append(moe_models.kl_divergence(gate_probabilities_high_T, torch.mean(gate_probabilities_high_T, dim = 0).repeat(len(gate_probabilities_high_T),1)).cpu().numpy())
