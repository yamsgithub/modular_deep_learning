...
#SBATCH --time=03:00:00  # Run for a max of 2 day

# Node resources:
# (choose between 1-4 gpus per node)

#SBATCH --partition=gpu    # Choose either "gpu" or "infer" node type        
#SBATCH --nodes=5 --ntasks-per-node=1 # Resources from a single node
...
python aaai_2022/src/cifar10/cifar10_train_from_model_importance_model.py -i $SLURM_ARRAY_TASK_ID
